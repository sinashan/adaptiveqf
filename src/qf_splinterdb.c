...
int qfdb_insert(QFDB *qfdb, uint64_t key, uint64_t count) {
    ...
    int ret = qf_insert_using_ll_table(qfdb->qf, hash, count, &result, QF_NO_LOCK | QF_KEY_IS_HASH);
...
int qfdb_query(QFDB *qfdb, uint64_t key) {
  ...
  // }
    ...
    uint64_t ret_hash = 0;
    int minirun_rank = qf_query_using_ll_table(qfdb->qf, hash, &ret_hash, QF_NO_LOCK | QF_KEY_IS_HASH);
